...
tar -czf configs.tar.gz $(ls $(echo "$HOME"/{.pythonrc,.p10k.zsh,.zshrc,.config/{plasmarc,spotifyd/spotifyd.conf,terminator/config,sublime-text-3/Packages/User/SyncSettings.sublime-settings,mps-youtube/config},.local/share/konsole/{default.dark.colorscheme,Shell.profile}}) 2>/dev/null)
